...
# fi
conda install -S -y --prefix /usr/local -c "${CHANNEL}" -c conda-forge habitat-sim=0.1.7 headless withbullet "python=${PYTHON_VERSION}" "numpy=${NUMPY_VERSION}" "pillow=${PIL_VERSION}" "cffi=${CFFI_VERSION}" "scipy=${SCIPY_VERSION}" "numba=${NUMBA_VERSION}"

# Shallow GIT clone for speed
git clone https://github.com/facebookresearch/habitat-lab.git -b v0.1.7 --depth 1
git clone https://github.com/facebookresearch/habitat-sim.git -b v0.1.7 --depth 1
...
rm -rf habitat_sim/ # Deletes the habitat_sim folder so it doesn't interfere with import path(v0.1.7)

# Download Assets(latest)
# python src_python/habitat_sim/utils/datasets_download.py --uids ci_test_assets --replace --data-path data/
# Download Assets(v0.1.7)
