...
def create_maze(maze_width=21, maze_height=21, create=False, seed=0, 
  ...
  if not create:
    if ((maze_width, maze_height)==(21,21)) | ((maze_width, maze_height)==(41,41)):
      np.load(DIR+'/maze_array_h{}_w{}.npy'.format(maze_width, maze_height))
      maze_array = np.load(DIR+'/maze_array_h{}_w{}.npy'.format(maze_width,maze_height))
...
class AudioVisualMazeEnv():
    ...
    def make_distance_bins(self,n_div_sound):
      ...
      return np.linspace(1, M, n_div_sound-1)
    ...
    def step(self,action_label):
        ...
        if action_label == "UP":           
          ...
          Y_new = Y + 1 if Y < self.maze_array.shape[0]-1 else Y
          X_new = X
          ...
          Y_new = Y
          X_new = X +1 if X < self.maze_array.shape[1]-1 else X
